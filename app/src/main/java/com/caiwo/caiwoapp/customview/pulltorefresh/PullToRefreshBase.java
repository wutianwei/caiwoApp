...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
		...
		case MotionEvent.ACTION_UP:
			if (mIsHandledTouchEvent) {
				...
				if (isReadyForPullDown()) {
					mPullUpState = ILoadingLayout.State.NONE;
					// 调用刷新
	...
	public void doPullRefreshing(final boolean smoothScroll, final long delayMillis) {
		postDelayed(new Runnable() {
			...
			public void run() {
				...
				int duration = smoothScroll ? (SCROLL_DURATION + 900): 0;
	 ...
	 * @return 返回值时间为毫秒
	 */
	protected long getSmoothScrollDuration2() {
		return 1000;
	}
	...
	protected void pullHeaderLayout(float delta) {
		...
		if(oldScrollY == 0){
			mFooterLayout.show(false);// false
		}
